...
/// Manages in-app dispatch voice calls on the rider dispatch screen.
class DeliveryCallUiController with WidgetsBindingObserver {
  ...
  void attach({
    ...
    final local = riderUid.trim();
    final remote = driverId.trim();
  ...
  Future<void> startOutgoingCall({
    ...
    final driverId = DeliveryStateMachine.canonicalAssignedDriverId(delivery);
    if (user == null ||
  ...
  Future<void> _endCall() async {
    ...
    try {
      ...
      debugPrint(
        'CALL_END_TAP rideId=${session.rideId} role=rider',
      );
      await callService.endCallFromUserTap(
        rideId: session.rideId,
        endedByUid: localUid,
