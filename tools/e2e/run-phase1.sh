...
preflight_ports() {
  local blocked=0
  for spec in "9098:auth" "8081:firestore" "9001:database" "5002:functions" "9199:flutterwave-stub"; do
    local port="${spec%%:*}"
    local name="${spec##*:}"
    if lsof -nP -iTCP:"$port" -sTCP:LISTEN >/dev/null 2>&1; then
      echo "e2e: warning — port $port ($name) already in use; stop stale processes first" >&2
      lsof -nP -iTCP:"$port" -sTCP:LISTEN 2>/dev/null | head -3 >&2 || true
      blocked=1
    fi
  done
  if [[ "$blocked" == "1" ]]; then
    echo "e2e: attempting to stop stale emulator listeners..." >&2
  ...
  if [[ "$blocked" == "1" ]]; then
    echo "e2e: hint — pkill -f 'firebase.*emulators' or kill stale java/node PIDs, then retry" >&2
    exit 1
  fi
}
...
FLW_STUB_PID=""
stop_flutterwave_stub() {
  if [[ -n "${FLW_STUB_PID:-}" ]]; then
    kill "${FLW_STUB_PID}" 2>/dev/null || true
    wait "${FLW_STUB_PID}" 2>/dev/null || true
    FLW_STUB_PID=""
  fi
}
trap stop_flutterwave_stub EXIT

echo "e2e: starting Flutterwave stub (${E2E_PROJECT_ID})..."
node tools/e2e/mocks/flutterwave-stub.mjs &
FLW_STUB_PID=$!
sleep 1
if ! kill -0 "${FLW_STUB_PID}" 2>/dev/null; then
  echo "e2e: Flutterwave stub failed to start" >&2
  exit 1
fi

echo "e2e: starting Firebase emulators (project=${E2E_PROJECT_ID}) for Phase 1..."
...
bash tools/e2e/install-functions-env.sh \
  ...
  "cd tools/e2e && npm run test:rtdb-repro && npm run test:phase1"
